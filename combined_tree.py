...
from zodiac_lines import SIGNS as ZODIAC_SIGNS

CSV_FILE = os.path.join(os.path.dirname(__file__), "combined_tree1.csv")
...
COMBINED_PARTS = [
    ...
    ("causal diagram", ZODIAC_SIGNS[14]),
    ("twin triangles", ZODIAC_SIGNS[15]),
]

COLORS = [
    (255, 80, 80), (255, 180, 0), (80, 255, 80), (0, 200, 255),
    (200, 80, 255), (255, 100, 200), (80, 255, 200), (255, 140, 40),
    (100, 100, 255), (255, 220, 120), (180, 255, 80),
]
...
def draw_part(surface, index, color, cx, cy, scale):
    for polyline in COMBINED_PARTS[index][1]:
...
def draw_tree(surface, part_indices, font, binary=None):
    ...
    for i, (parent, x, y) in enumerate(TREE_LAYOUT):
        ...
        lbl = font.render(COMBINED_PARTS[idx][0], True, color)
...
def new_parts():
    return random.choices(range(len(COMBINED_PARTS)), k=len(TREE_LAYOUT))
...
def main():
    ...
    pygame.display.set_caption("Combined Tree")
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                for i, (_, nx, ny) in enumerate(TREE_LAYOUT):
                    if math.hypot(mx - nx, my - ny) <= SCALE:
                        ...
                        mouse_pos = f"{COMBINED_PARTS[parts[i]][0]} ({mx}, {my})"
                        break
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_r:
                    ...
                    path = os.path.join(os.path.dirname(__file__), f"combined_tree{n}.csv")
